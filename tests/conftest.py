...
def mock_filter_passthrough():
    """Patch FalsePositiveFilter.filter to return copies of the given results (no HTTP calls)."""
    async def passthrough(self, results):
        copied = []
        for result in results:
            result_copy = copy.copy(result)
            if result_copy.status == ResultStatus.FOUND:
                result_copy.http_status = 200
            copied.append(result_copy)
        return copied

    with patch(
        "processing.filter.FalsePositiveFilter.filter",
        new=passthrough
    ):
        yield
